...
public class DDTeleporter {
    ...
    private static boolean checkDestination(WorldServer world, Point4D destination, int orientation) {
        ...
        if (blockBottom != null) {
            if (!blockBottom.isReplaceable(world, point.getX(), point.getY(), point.getZ()) && blockBottom.isOpaqueCube()) {
                return false;
            }
        }
        if (blockTop != null) {
    ...
    private static void placeInPortal(Entity entity, WorldServer world, Point4D destination, boolean checkOrientation) {
        int x = destination.getX();
        int y = destination.getY();
        int z = destination.getZ();
        ...
        int orientation;
        if (checkOrientation) {
            orientation = getDestinationOrientation(destination);
    ...
    private static int getDestinationOrientation(Point4D door) {
    ...
    public static Entity teleportEntity(Entity entity, Point4D destination, boolean checkOrientation) {
        ...
        if (player != null) {
            ...
            FMLCommonHandler.instance().firePlayerChangedDimensionEvent(player, oldDim, destination.getDimension());
        }
        DDTeleporter.placeInPortal(entity, newWorld, destination, checkOrientation);
        return entity;
    }
    ...
    /**
     * Primary function used to teleport the player using doors. Performs numerous null checks, and also generates the destination door/pocket if it has not
     * done so already. Also ensures correct orientation relative to the door.
     *
    ...
    private static boolean initializeDestination(DimLink link, Entity entity, Block door) {
        ...
        switch (link.linkType()) {
            ...
            case POCKET:
                return PocketBuilder.generateNewPocket(link, door, DimensionType.POCKET);
            case PERSONAL:
    ...
    private static boolean setupPersonalLink(DimLink link, Entity player, Block door) {
        if (!(player instanceof EntityPlayer)) {
            return false;
        }

        NewDimData dim = PocketManager.getPersonalDimensionForPlayer(player.getCommandSenderName());
        if (dim == null) {
    ...
    private static boolean isValidForDungeonExit(NewDimData destination) {
        ...
        if (destination.id() == END_DIMENSION_ID || destination.id() == DDProperties.instance().LimboDimensionID) {
            return false;
        }
    ...
    private static boolean generateSafeExit(NewDimData destinationDim, DimLink link) {
        ...
        if (destination != null) {
            ...
            for (int dx = -1; dx <= 1; dx++) {
                for (int dz = -1; dz <= 1; dz++) {
                    ...
                    if (!world.getBlock(x + dx, y, z + dz).isNormalCube()) {
                        world.setBlock(x + dx, y, z + dz, DDBlocks.blockDimWall, 0, 2);
                    }
                }
            }
            ...
            orientation = BlockRotator.transformMetadata(orientation, 2, Block.getIdFromBlock(DDBlocks.warpDoor));
            ItemDoor.placeDoorBlock(world, x, y + 1, z, orientation, DDBlocks.warpDoor);
