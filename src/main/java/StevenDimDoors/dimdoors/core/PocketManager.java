...
import StevenDimDoors.dimdoors.helpers.Compactor;
import StevenDimDoors.dimdoors.helpers.DeleteFolder;
import StevenDimDoors.dimdoors.saving.DDSaveHandler;
import StevenDimDoors.dimdoors.saving.OldSaveImporter;
...
public class PocketManager {
    ...
    private static class InnerDimData extends NewDimData {
        // This class allows us to instantiate NewDimData indirectly without exposing a public constructor from NewDimData. It's meant to stop us from constructing
        // instances of NewDimData going through PocketManager. In turn, that enforces that any link destinations must be real dimensions controlled by PocketManager.

        InnerDimData(int id, InnerDimData parent, DimensionType type, IUpdateWatcher<ClientLinkData> linkWatcher) {
            super(id, parent, type, linkWatcher);
        }

        InnerDimData(int id, NewDimData root, DimensionType type) {
    ...
    private static class DimRegistrationCallback implements IDimRegistrationCallback {
        ...
        // dim data without having to instantiate a bunch of data containers and without exposing
        // an "unsafe" creation method for anyone to call. Integrity protection for the win!
        // It's like exposing a private constructor ONLY to a very specific trusted class.
    ...
    public static void load() {
        ...
        if (FMLCommonHandler.instance().getEffectiveSide().isClient()) {
            ...
            // Shouldn't try to load everything if we are a client
        ...
        // Register Limbo
        registerDimension(DDProperties.instance().LimboDimensionID, null, DimensionType.ROOT);
            ...
            if (dimension.isPocketDimension()) {
                try {
                    if (personalPocketsMapping.containsValue(dimension)) {
                        DimensionManager.registerDimension(dimension.id(), DDProperties.instance().PersonalPocketProviderID);
                    } else {
    ...
    private static NewDimData registerClientDimension(int dimensionID, int rootID, DimensionType type) {
        ...
        // SenseiKiwi: I'm a little worried about how createDimensionData will raise an event when it creates any root dimensions... Needs checking later.
    ...
    protected static NewDimData createDimensionData(int dimensionID) {
        ...
        // dimension, assume it's a non-pocket dimension that hasn't been initialized with us before and create a NewDimData instance for it.
    ...
    public static void writePacket(ByteBuf output) {
    ...
    public static void readPacket(ByteBuf input) {
