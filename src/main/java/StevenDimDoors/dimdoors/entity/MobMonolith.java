...
public class MobMonolith extends EntityFlying implements IMob {
    ...
    protected void applyEntityAttributes() {
        ...
        this.getAttributeMap().getAttributeInstance(SharedMonsterAttributes.maxHealth).setBaseValue(57005); // Who knows if this actually does what I think
    }
    ...
    protected void entityInit() {
        ...
        this.dataWatcher.addObject(AGGRO_WATCHER_INDEX, (short) 0);
    }
    ...
    public void onEntityUpdate() {
        ...
        if (!(this.worldObj.provider instanceof WorldProviderLimbo || this.worldObj.provider instanceof WorldProviderPocket)) {
            this.setDead();
        ...
        if (player != null) {
            ...
            if (!this.worldObj.isRemote && !(this.worldObj.provider instanceof WorldProviderLimbo)) {
            ...
            if (visibility) {
                ...
                if (aggro >= MAX_AGGRO && !this.worldObj.isRemote
                        && DDProperties.instance().MonolithTeleportationEnabled && !player.capabilities.isCreativeMode
                        && !(this.worldObj.provider instanceof WorldProviderLimbo)) {
                    this.aggro = 0;
    ...
    private void updateAggroLevel(EntityPlayer player, boolean visibility) {
        ...
        if (!this.worldObj.isRemote) {
            ...
            if (visibility) {
                if (this.worldObj.provider instanceof WorldProviderLimbo) {
                    aggro++;
                } else {
            ...
            this.dataWatcher.updateObject(AGGRO_WATCHER_INDEX, aggro);
        } else {
            // Client side...
    ...
    public boolean getCanSpawnHere() {
        ...
        } else if (this.worldObj.provider instanceof WorldProviderPocket) {
            if (list.size() > 5
