...
import StevenDimDoors.dimdoors.util.ChunkLocation;
...
public class CustomLimboPopulator implements IRegularTickReceiver {
        ...
        if (!locations.isEmpty()) {
            ...
            if (isMobSpawningAllowed()) {
                ...
                for (ChunkLocation location : locations) {
                    if (location.DimensionID == DDProperties.instance().LimboDimensionID) {
                        // Limbo chunk
                        ...
                        if (limboWorld == null) {
                            limboWorld = PocketManager.loadDimension(DDProperties.instance().LimboDimensionID);
                        }
    ...
    public static void registerChunkForPopulation(int dimensionID, int chunkX, int chunkZ) {
        ChunkLocation location = new ChunkLocation(dimensionID, chunkX, chunkZ);
        locations.add(location);
    }

    private static void placeMonolithsInPocket(int dimensionID, int chunkX, int chunkZ) {
        ...
        do {
            ...
            while ((block == DDBlocks.blockDimWall || block == DDBlocks.blockDimWallPerm) && y > 0) {
                y--;
                block = pocket.getBlock(x, y, z);
            }
            while (block == Blocks.air && y > 0) {
    ...
    private static void placeMonolithsInLimbo(World limbo, int chunkX, int chunkZ) {
        ...
        if (random.nextInt(MAX_MONOLITH_SPAWNING_CHANCE) < DDProperties.instance().MonolithSpawningChance) {
            int y = 0;
            int yTest;
            do {
                ...
                while (limbo.getBlock(x, y, z) == Blocks.air && y < 255) {
                    y++;
                }
