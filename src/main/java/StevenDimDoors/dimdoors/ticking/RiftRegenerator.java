...
public class RiftRegenerator implements IRegularTickReceiver {
    ...
    private void regenerateRift(Point4D location) {
        ...
        if (DDBlocks.blockRift.isModBlockImmune(world, x, y, z)) {
            return;
        }

        // If the location is occupied by an immune block, then reschedule.
        if (DDBlocks.blockRift.isBlockImmune(world, x, y, z)) {
            ...
            if (world.setBlock(x, y, z, DDBlocks.blockRift)) {
                DDBlocks.blockRift.dropWorldThread(block, world, x, y, z, random);
            }
        }
    }
}
