...
public class MazeBuilder {
    ...
    private static void buildBox(World world, Point3D offset, Point3D minCorner, Point3D maxCorner, Block b, int metadata) {
        ...
        for (x = minX; x <= maxX; x++) {
            for (z = minZ; z <= maxZ; z++) {
                setBlockDirectly(world, x, minY, z, b, metadata);
                setBlockDirectly(world, x, maxY, z, b, metadata);
            }
        }
        for (x = minX; x <= maxX; x++) {
            for (y = minY; y <= maxY; y++) {
                setBlockDirectly(world, x, y, minZ, b, metadata);
                setBlockDirectly(world, x, y, maxZ, b, metadata);
            }
        }
        for (z = minZ; z <= maxZ; z++) {
            for (y = minY; y <= maxY; y++) {
                setBlockDirectly(world, minX, y, z, b, metadata);
                setBlockDirectly(world, maxX, y, z, b, metadata);
            }
        }
    }

    private static void setBlockDirectly(World world, int x, int y, int z, Block b, int metadata) {
        if (b == null) {
            return;
        }
        ...
        extBlockStorage.func_150818_a(localX, y & 15, localZ, b);
